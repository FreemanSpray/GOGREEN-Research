...
class GOGREEN:
    ...
    def MSRfit(self, data:list, useLog:list=[False, False], axes:list=None, row:int=None, col:int=None, allData:bool=False, useMembers:str='only', additionalCriteria:list=None, useStandards:bool=True, typeRestrict:str=None, color:str=None, bootstrap:bool=True):
        """
    ...
    def getRatio(self, x:float=None, y:float=None, bootstrap:bool=True) -> list:
        """
        ...
        mMemberQ, bMemberQ = self.MSRfit(data=None, useLog=[True, True], allData=True, useMembers='only', typeRestrict='Quiescent', bootstrap=bootstrap)
        mMemberSF, bMemberSF = self.MSRfit(data=None, useLog=[True, True], allData=True, useMembers='only', typeRestrict='Star-Forming', bootstrap=bootstrap)
        mNonMemberQ, bNonMemberQ = self.MSRfit(data=None, useLog=[True, True], allData=True, useMembers='not', typeRestrict='Quiescent', color='orange', bootstrap=bootstrap)
        mNonMemberSF, bNonMemberSF = self.MSRfit(data=None, useLog=[True, True], allData=True, useMembers='not', typeRestrict='Star-Forming', color='green', bootstrap=bootstrap)
        plt.xlim(9.5, 11.5)
        plt.ylim(-0.75, 1.25)
        plt.legend()
        if x != None:
    ...
    def plotUnwrapped(self, xQuantityName:str, yQuantityName:str, additionalCriteria:list=None, colorType:str=None, useStandards:bool=True, useLog:list=[False,False], fitLine:bool=False,
        data:pd.DataFrame=None, color1:list=None, color2:list=None, plot=None, axes:list=None, row:int=None, col:int=None, holdLbls:bool=False, bootstrap:bool=True, plotErrBars:bool=False,):
            """
        ...
        # Plot all clusters on the same plot            
            ...
            for clusterName in self._structClusterNames:
                ...
                if (clusterName != self._structClusterNames[-1]):
                    x, y = self.plotUnwrapped(xQuantityName, yQuantityName, additionalCriteria, colorType, useStandards, useLog, False, data, color1, color2, plt, axes=None, row=None, col=None, holdLbls=True, bootstrap=bootstrap, plotErrBars=plotErrBars)
                # Only add legend labels for the last plot otherwise the lengend will be filled with multiple duplicates of these labels
                else:
                    x, y = self.plotUnwrapped(xQuantityName, yQuantityName, additionalCriteria, colorType, useStandards, useLog, False, data, color1, color2, plt, axes=None, row=None, col=None, holdLbls=False, bootstrap=bootstrap, plotErrBars=plotErrBars)
                # Update data count totals
                xTot+=x
                yTot+=y
            # generate best fit line
            if fitLine == True:
                ...
                if colorType == 'passive':
                    self.MSRfit([], useLog, allData=True, useMembers=useMembers, additionalCriteria=additionalCriteria, useStandards=useStandards, typeRestrict='Quiescent', color=color1, bootstrap=bootstrap)
                    self.MSRfit([], useLog, allData=True, useMembers=useMembers, additionalCriteria=additionalCriteria, useStandards=useStandards, typeRestrict='Star-Forming', color=color2, bootstrap=bootstrap)
                # In the case of plotting elliptical vs spiral inclined galaxies (based on Sersic index), we plot two separate fit lines NOTE: Handling of these cases not yet implemented in MSRfit()
                    ...
                    self.MSRfit([], useLog, allData=True, useMembers=useMembers, additionalCriteria=additionalCriteria, useStandards=useStandards, typeRestrict='Elliptical', color=color1, bootstrap=bootstrap)
                    self.MSRfit([], useLog, allData=True, useMembers=useMembers, additionalCriteria=additionalCriteria, useStandards=useStandards, typeRestrict='Spiral', color=color2, bootstrap=bootstrap)
                else:
                    self.MSRfit([], useLog, allData=True, useMembers=useMembers, additionalCriteria=additionalCriteria, useStandards=useStandards, bootstrap=bootstrap)
        else:
