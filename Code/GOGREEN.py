...
class GOGREEN:
    ...
    def MSRfit(self, data:list, useLog:list=[False, False], axes:list=None, row:int=None, col:int=None, allData:bool=False, onlyMembers:bool=True, additionalCriteria:list=None, useStandards:bool=True):
        """
        ...
        if allData:
            data = pd.DataFrame() #setting an initial value to append to.
            for clusterName  in self._structClusterNames:
        ...
        size = self.reConvert(data) #converting all effective radii from units of arcsec to kpc using their spectroscopic redshifts
        ...
        for i in range(0, len(mass)):
            if np.isnan(mass[i]) == True: #checking if there are any remaining missing values (because there is no Mstellar)
                mass[i] = 1 #setting to arbitrary value so polyfit will not fail (is there a way to simply exclude these galaxies?). NOTE: there is a way to do this in the notebook rather than in the class. Find it.
    ...
    def plot(self, xQuantityName:str, yQuantityName:str, plotType:int, clusterName:str=None, additionalCriteria:list=None, onlyMembers:bool=True, colorType:str=None,
        ...
        # Plot all clusters on the same plot            
            ...
            # generate best fit line
            if fitLine == True:
                self.MSRfit([], useLog, allData=True, onlyMembers=onlyMembers, additionalCriteria=additionalCriteria, useStandards=useStandards)
        else:
