...
class GOGREEN:
    ...
    def fit(self, data:list, fitX:str, fitY:str, useLog:list):
        """
        ...
        :param fitX:   Used to change the column who values are used as the x for the fit line.
                        Default: None
        :param fitY:   Used to change the column who values are used as the y for the fit line.
                        Default: None
        :param useLog: Used to determine which axes should be displayed in log format. Only applies to x if fitX is not None and y if fitY is note None.
                        Default: [False, False]
        """

        dist = data['re'].values * (cosmo.kpc_proper_per_arcmin(data['zspec'].values)/60)
        #print(len(dist))
        #print(len(data['re'].values))
        #print(dist[0])
        #print(data['re'].values[0])
        #print(type(data['re'].values[0]))
        #print(type(dist[0]))
        xFitData = np.log10(data['Mstellar'].values)
        yFitData = np.log10(dist)
        #yFitData = np.log10(data['re'].values)
        if fitX != None:
            xFitData = data[fitX].values
            if useLog[0] == True:
                xFitData = np.log10(data[fitX].values)
        if fitY != None:
            yFitData = data[fitY].values
            if useLog[1] == True:
                yFitData = np.log10(data[fitY].values)
        m, b = np.polyfit(xFitData, yFitData, 1)
        plt.plot(xFitData, m * xFitData + b)
    # END _FIT
    ...
    def plot(self, xQuantityName:str, yQuantityName:str, plotType:int, clusterName:str=None, additionalCriteria:list=None, onlyMembers:bool=True, colorType:str=None,
             colors:list=None, useStandards:bool=True, xRange:list=None, yRange:list=None, xLabel:str='', yLabel:str='', useLog:list=[False,False], fitLine:bool=False, fitX:str=None, fitY:str=None):
        """
        ...
        if plotType == 1:
            ...
            if colorType == None:
                ...
                if fitLine == True:
                    self.fit(data, fitX, fitY, useLog)
