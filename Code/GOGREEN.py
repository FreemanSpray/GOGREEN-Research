...
class GOGREEN:
    ...
    def MSRfit(self, data:list, axes:list=None, row:int=None, col:int=None):
        """
        fit (private method) fit a best fit line to data generated by the plot() method
        ...
        size = data['re'].values * (cosmo.kpc_proper_per_arcmin(data['zspec'].values)/60) #converting all effective radii from units of arcsec to kpc using their spectroscopic redshifts
        for i in range(0, len(size)):
            if np.isnan(size[i]) == True: #checking where conversion failed due to lack of zspec value
                size[i] = data['re'].values[i] * (cosmo.kpc_proper_per_arcmin(data['zphot'].values[i])/60) #use photometric redshifts instead where there are no spectroscopic redshifts
            if np.isnan(size[i]) == True: #checking if there are any remaining missing values (either because there is no redshift or there is no re)
                size[i] = 1 #setting to arbitrary value so polyfit will not fail (is there a way to simply exclude these galaxies?).  NOTE: arbitrary value CANNOT be 0.
        size = (size / u.kpc) * u.arcmin # removing units so the data can be used in the functions below

        mass = data['Mstellar'].values
        for i in range(0, len(mass)):
            if np.isnan(mass[i]) == True: #checking if there are any remaining missing values (because there is no Mstellar)
                mass[i] = 1 #setting to arbitrary value so polyfit will not fail (is there a way to simply exclude these galaxies?). NOTE: arbitrary value CANNOT be 0.
        ...
        # Plot all clusters on the same plot            
            ...
            for clusterName in self._structClusterNames:
                ...
                if colorType == None:
                    ...
                    # generate best fit line NOTE: generates for each cluster - would we rather only have one? Data will need to be adjusted if so.
                    if fitLine == True:
                        self.MSRfit(data)
                elif colorType == 'membership':
                    # Extract desired quantities from data
